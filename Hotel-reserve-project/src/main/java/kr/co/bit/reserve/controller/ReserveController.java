...
public class ReserveController {
	...
	public String reserveForm(HttpServletRequest request, Model model){
		...
		if (_y != null)
			y = Integer.parseInt(_y);			// 넘어온 값이 있으면 y(년도) 수정
		if (_m != null)
			m = Integer.parseInt(_m);			// 넘어온 값이 있으면 m(월) 수정
		...
		model.addAttribute("ystr", ystr);			// 년도를 String으로
		model.addAttribute("mstr", mstr);			// 월을 String으로
		model.addAttribute("mlength", mlength);		// 월의 길이
